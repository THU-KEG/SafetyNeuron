...
def convert_sharegpt_data(data_dir, output_dir, data_file="sharegpt_html_cleaned_and_split_4096.json", num_examples=None):
    ...
    output_path = os.path.join(output_dir, "sharegpt_4096_data.jsonl")
    with open(output_path, "w") as fout:
        invalid_cnt = 0
        for idx, example in enumerate(examples):
            messages = []
...
def convert_hh_rlhf_harmless_data(data_dir, output_dir, num_examples=None):
    ...
    for split in ['train', 'test']:
        ...
        if num_examples:
            examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"hh_rlhf_harmless_data_{split}.jsonl")
        with open(output_path, "w") as fout:
            for idx, example in enumerate(examples):
                messages = []
                # if '[]' in example['chosen']:
                    # example['chosen'] = example['chosen'].replace('[]', '<>')
                    # print(example['chosen'])
                for pair in example['chosen'].split('Human:')[1:]:
...
def convert_hh_rlhf_helpful_data(data_dir, output_dir, num_examples=None):
    ...
    for split in ['train', 'test']:
        ...
        if num_examples:
            examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"hh_rlhf_helpful_data_{split}.jsonl")
        with open(output_path, "w") as fout:
            for idx, example in enumerate(examples):
                messages = []
                # if '[]' in example['chosen']:
                ...
                fout.write(json.dumps({
                    "dataset": "hh_rlhf_helpful",
                    "id": f"helpful_{idx}",
                    "messages": messages,
                }) + "\n")

def convert_BeaverTails_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    for split in ['train', 'test']:
        examples = []
        with open(os.path.join(data_dir, f"{split}.jsonl"), "r") as fin:
        ...
        if num_examples:
            examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"beavertails_data_{split}.jsonl")
        with open(output_path, "w") as fout:
            for idx, example in enumerate(examples):
                if example['is_safe']:
                    messages = []
...
def convert_HarmfulQA_data(data_dir, output_dir, data_file='harmful_qa_2048.json', num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, data_file), "r") as fin:
        examples.extend(json.load(fin))
    ...
    output_path = os.path.join(output_dir, "harmfulqa_data.jsonl")
    with open(output_path, "w") as fout:
        invalid_cnt = 0
        for idx, example in enumerate(examples):
            messages = []
            ...
            if messages and valid:
                fout.write(json.dumps({
                    "dataset": "harmfulqa",
                    "id": f"harmfulqa_{example['id']}",
                    "messages": messages
                }) + "\n")
        if invalid_cnt > 0:
            print(f"# of invalid examples in harmfulqa data: {invalid_cnt}")
